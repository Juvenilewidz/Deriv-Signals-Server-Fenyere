...
# -------------------------
def get_trend_bias(price, ma3_val):
    """Simple trend bias using MA3 filter"""
    if ma3_val is None:
        return "UNDEFINED"
    
    if price > ma3_val:
        return "BUY_BIAS"  # Above MA3 = Buy bias
    elif price < ma3_val:
        return "SELL_BIAS"  # Below MA3 = Sell bias
    else:
        return "NEUTRAL"
...
def detect_signal(candles, tf, shorthand):
    """Simple DSR signal detection - no missed signals"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # Get trend bias (simple)
    trend_bias = get_trend_bias(current_close, current_ma3)
    ...
    # Check if rejection occurred near MA levels
    ...
    # BUY SIGNAL: Buy bias + rejection near MA support
    if trend_bias == "BUY_BIAS":
        ...
        elif near_ma_high:  # Even high touching MA in uptrend
            signal_side = "BUY"
            ma_level = ma_level_high
    
    # SELL SIGNAL: Sell bias + rejection near MA resistance
        ...
        if near_ma_high:
            ...
            signal_side = "SELL"
            ma_level = ma_level_close
        elif near_ma_low:  # Even low touching MA in downtrend
            signal_side = "SELL"
            ma_level = ma_level_low
    
    if signal_side:
        if DEBUG:
            print(f"DSR Signal: {signal_side} - {pattern_type} at {ma_level} (Bias: {trend_bias})")

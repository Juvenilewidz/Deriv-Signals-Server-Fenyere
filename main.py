...
# DATA FETCHING (COMPLETED CANDLES ONLY) with 1s optimization
def fetch_completed_candles_only(sym, tf, count=CANDLES_N):
    """Fetch candles ensuring we only work with completed ones - optimized for 1s"""
    # Reduce candle count for 1s timeframes to avoid timeouts
    if tf == 1:
        count = min(count, 250)  # Reduced count for 1s indices
        timeout = 25  # Longer timeout for 1s data
    else:
        timeout = 20
    ...
    for attempt in range(3):
        try:
            ...
            if "candles" in response and len(response["candles"]) > 2:
                ...
                if DEBUG:
                    print(f"Using {len(completed_candles)} completed candles for {sym} ({tf}s)")
                
                return completed_candles
        
        except Exception as e:
            if DEBUG:
                print(f"Attempt {attempt + 1} failed for {sym} ({tf}s): {e}")
            time.sleep(0.5 if tf == 1 else 1)  # Faster retry for 1s
    
    return []

# MASTER SIGNAL DETECTION (IRONCLAD RULES) with 1s adaptation
def detect_signal_ironclad(candles, tf, shorthand):
    """Ironclad signal detection with absolute rule enforcement + 1s optimization"""
    n = len(candles)
    
    # Adaptive minimum candles based on timeframe
    min_candles_required = 150 if tf > 1 else 120  # Lower requirement for 1s
    ...
    # ADAPTIVE PARAMETERS FOR 1s INDICES
    if tf == 1:
        # More relaxed parameters for high-frequency analysis
        global MA_SEPARATION_MINIMUM, SWING_MINIMUM, CONSOLIDATION_THRESHOLD
        MA_SEPARATION_MINIMUM = 0.005      # Slightly relaxed for 1s volatility
        SWING_MINIMUM = 0.010              # Reduced swing requirement for 1s
        CONSOLIDATION_THRESHOLD = 0.003    # Tighter consolidation detection for 1s
...
# MAIN EXECUTION with 1s integration
def run_ironclad_analysis():
    """Main analysis with ironclad rule enforcement + 1s integration"""
    validated_signals = 0
    ...
    for shorthand, deriv_symbol in SYMBOL_MAP.items():
        try:
            ...
            if DEBUG:
                print(f"IRONCLAD analysis for {shorthand} on {tf}s timeframe...")
            
            candles = fetch_completed_candles_only(deriv_symbol, tf)
            min_required = 150 if tf > 1 else 120
            
            if len(candles) < min_required:
                if DEBUG:
                    print(f"{shorthand}: Insufficient data ({len(candles)} candles)")
                continue
            ...
            # Create alert with timeframe-specific formatting
            if tf == 1:
                tf_display = "1s ⚡"
            elif tf < 60:
                tf_display = f"{tf}s"
            else:
                tf_display = f"{tf//60}m"
            ...
            if tf == 1:
                caption += f"⚡ HIGH-FREQUENCY SIGNAL - 1s Analysis\n"
            
            caption += (f"📝 Status: IRONCLAD VALIDATION PASSED\n"
            ...
            if success:
                ...
                validated_signals += 1
                freq_label = "1s HIGH-FREQ" if tf == 1 else "STANDARD"
                if DEBUG:
                    print(f"✅ {freq_label} VALIDATED signal sent: {shorthand} {signal['side']}")
    ...
    if DEBUG:
        print(f"Analysis complete. {validated_signals} ironclad validated signals found (including 1s).")

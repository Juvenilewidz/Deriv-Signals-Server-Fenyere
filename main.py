#!/usr/bin/env python3
"""
main.py — Dynamic Support & Resistance (EXACTLY AS REQUESTED)

SIMPLE RULES - NO DEVIATIONS:
1. MA3 break (up or down) 
2. Price retests MA1 or MA2
3. Rejection candlestick forms at retest
4. Signal fires IMMEDIATELY at rejection candle close
5. NO confirmation candles, NO counting, NO delays
"""
...
# STEP 1: FIND MA3 BREAK
def find_ma3_break(candles, ma3, current_idx):
    """Find any MA3 break in recent candles"""
    for i in range(max(1, current_idx - MA3_LOOKBACK), current_idx):
...
def detect_simple_signal(candles, tf, shorthand):
    ...
    # STEP 1: Find MA3 break
    ma3_break_direction = find_ma3_break(candles, ma3, current_idx)
    if not ma3_break_direction:
        return None
    
    # STEP 2: Check if retesting MA1/MA2  
    ...
    signal_side = "BUY" if ma3_break_direction == "UP" else "SELL"

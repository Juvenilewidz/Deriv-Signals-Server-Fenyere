...
def detect_signal(candles, tf, shorthand):
    """ANTI-SPAM DSR - Only genuine setups as per strategy description"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # FILTER 1: MA arrangement must be PERFECT
    ma_arrangement = get_ma_arrangement(current_ma1, current_ma2, current_ma3)
    if ma_arrangement not in ["BULLISH_ARRANGEMENT", "BEARISH_ARRANGEMENT"]:
        return None
    
    # FILTER 2: Strict MA1/MA2 relationship
    can_sell = (ma_arrangement == "BEARISH_ARRANGEMENT" and current_ma1 < current_ma2)
    can_buy = (ma_arrangement == "BULLISH_ARRANGEMENT" and current_ma1 > current_ma2)
    
    if not (can_buy or can_sell):
        return None
    
    # FILTER 3: NO ranging markets
    is_ranging = check_ranging_market(candles, ma1, ma2, current_idx)
    if is_ranging:
        return None
    
    # FILTER 4: Must have rejection pattern
    is_rejection, pattern_type = is_rejection_candle(current_candle)
    if not is_rejection:
        return None
    
    # FILTER 5: Price must be DIRECTLY interacting with MA1 or MA2
    # Check if ANY part of the candle touched the MA levels
    touched_ma1_high = abs(current_high - current_ma1) <= current_ma1 * 0.001
    touched_ma1_low = abs(current_low - current_ma1) <= current_ma1 * 0.001
    touched_ma1_close = abs(current_close - current_ma1) <= current_ma1 * 0.001
    
    touched_ma2_high = abs(current_high - current_ma2) <= current_ma2 * 0.001
    touched_ma2_low = abs(current_low - current_ma2) <= current_ma2 * 0.001
    touched_ma2_close = abs(current_close - current_ma2) <= current_ma2 * 0.001
    
    touched_ma1 = touched_ma1_high or touched_ma1_low or touched_ma1_close
    touched_ma2 = touched_ma2_high or touched_ma2_low or touched_ma2_close
    
    if not (touched_ma1 or touched_ma2):
        if DEBUG:
            print(f"BLOCKED: {shorthand} price not touching MA1/MA2 directly")
        return None
    
    # FILTER 6: Signal direction must match MA bias
    signal_side = None
    ma_level = "NONE"
    
    if can_buy and (touched_ma1 or touched_ma2):
        # Only BUY if touching MA in uptrend context
        if touched_ma1:
            signal_side = "BUY"
            ma_level = "MA1"
        elif touched_ma2:
    ...
    elif can_sell and (touched_ma1 or touched_ma2):
        # Only SELL if touching MA in downtrend context
        if touched_ma1:
            signal_side = "SELL"
            ma_level = "MA1"
        elif touched_ma2:
    ...
    # FILTER 7: Final validation - must make logical sense
    if not signal_side:
        return None
    
    # FILTER 8: Cooldown period - no signals within 30 minutes of last signal
    last_signal_time = getattr(detect_signal, f'last_signal_{shorthand}', 0)
    current_time = current_candle["epoch"]
    
    if current_time - last_signal_time < 1800:  # 30 minutes cooldown
        if DEBUG:
            print(f"BLOCKED: {shorthand} cooldown period active")
        return None
    
    # Mark this signal time
    setattr(detect_signal, f'last_signal_{shorthand}', current_time)
    
    if DEBUG:
        print(f"VALID DSR SIGNAL: {signal_side} - {pattern_type} at {ma_level} - MA arrangement: {ma_arrangement}")
    ...
    return {
        ...
        "crossover": "NONE",
        "context": f"MA1 {'above' if can_buy else 'below'} MA2 - {'up' if can_buy else 'down'}trend confirmed",
        "price": current_close,
        "ma1": current_ma1,
        "ma2": current_ma2, 
        "ma3": current_ma3,

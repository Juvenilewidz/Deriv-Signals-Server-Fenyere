...
def analyze_and_notify():
    ...
    # collect last rejection candidate per (symbol, tf)
    ...
    for symbol in ASSETS:
        ...
        for tf in TIMEFRAMES:
            ...
            # keep the last rejection candidate for this (symbol,tf)
            if direction is None and reason and reason != "no candles":
    ...
    # --- send one short rejection chart per (symbol,tf) ---
    sent_rejection_keys = set()
    for (symbol, tf), (candles, reason) in last_rejections.items():
        # skip if this symbol got a real accepted signal this run
        if symbol in last_sent_signal_by_symbol and last_sent_signal_by_symbol[symbol] is not None:
            continue
        if (symbol, tf) in sent_rejection_keys:
            continue
        ...
        try:
            ...
            chart_path = make_chart(candles, ma1, ma2, ma3, i_rej_chart, "REJECTED", symbol, tf, last_n=60)
        except Exception as e:
            log("Failed preparing rejection chart for", symbol, tf, e)
            chart_path = None

        # short, friendly reason (no long dumps)
        ...
        if chart_path and TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
            try:
                ...
                log("send rejection photo error:", e)
                sent_ok = False
            try:
                os.unlink(chart_path)
            except Exception:
            ...
            # fallback text-only if photo not possible
            try:
                if send_telegram_message:
                    ...
                    resp = requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                                         data={"chat_id": TELEGRAM_CHAT_ID, "text": caption}, timeout=15)
                    sent_ok = (resp.status_code == 200)
            except Exception as e:
                log("send rejection text error:", e)
                sent_ok = False

        if sent_ok:
            sent_rejection_keys.add((symbol, tf))
            log("Sent rejection for", symbol, tf, short_reason)

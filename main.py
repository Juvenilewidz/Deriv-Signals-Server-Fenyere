...
def detect_signal(candles, tf, shorthand):
    """DSR with MA crossover and arrangement-based trend confirmation"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # CORE DSR LOGIC: MA arrangement determines trend bias
    ma_arrangement = get_ma_arrangement(current_ma1, current_ma2, current_ma3)
    
    # CRITICAL CONDITIONS based on your description:
    # SELL BIAS: Only when MA1 is BELOW MA2 (bearish arrangement)
    # BUY BIAS: Only when MA1 is ABOVE MA2 (bullish arrangement)
    
    can_sell = (ma_arrangement == "BEARISH_ARRANGEMENT" and 
                current_ma1 < current_ma2)  # MA1 below MA2
    
    can_buy = (ma_arrangement == "BULLISH_ARRANGEMENT" and 
               current_ma1 > current_ma2)   # MA1 above MA2
    
    if not (can_buy or can_sell):
        if DEBUG:
            print(f"No valid MA arrangement for {shorthand}: {ma_arrangement}")
        return None
    
    # Detect MA crossovers (keep 3 eyes on crossovers as you said)
    has_crossover, crossover_type = detect_ma_crossover(ma1, ma2, current_idx)
    
    # Check for ranging: If price touches MA2 more than twice = ranging
    ...
    # Check if rejection occurred near MA levels (MA1/MA2 dynamic S/R)
    ...
    # BUY SIGNAL: Bullish arrangement (MA1 > MA2) + rejection at MA support
    if can_buy:
        if near_ma_low:
            signal_side = "BUY"
            ma_level = ma_level_low
            signal_context = "MA1 above MA2 - uptrend confirmed"
        elif near_ma_close:
            signal_side = "BUY"
    ...
    # SELL SIGNAL: Bearish arrangement (MA1 < MA2) + rejection at MA resistance
    elif can_sell:
        if near_ma_high:
            signal_side = "SELL"
            ma_level = ma_level_high
            signal_context = "MA1 below MA2 - downtrend confirmed"
        elif near_ma_close:
            signal_side = "SELL"
            ma_level = ma_level_close
            signal_context = "MA1 below MA2 - downtrend confirmed"
    ...
    if signal_side:
        if DEBUG:
            crossover_info = f" (Recent {crossover_type})" if has_crossover else ""
            print(f"DSR Signal: {signal_side} - {pattern_type} at {ma_level} - {signal_context}{crossover_info}")

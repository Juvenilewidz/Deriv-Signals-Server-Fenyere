...
# -------------------------
# Core DSR Signal Detection (Enhanced)
# -------------------------
def detect_signal(candles, tf, shorthand):
    """Complete DSR Strategy - Enhanced with proper conditions"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # DSR RULE 2 & 3: Price position requirements (NO TOLERANCE FILTERS)
    ...
    # DSR RULE 5: Check if candle touched MA1 or MA2 (NO TOLERANCE)
    touched_ma1 = (current_low <= current_ma1 <= current_high)
    touched_ma2 = (current_low <= current_ma2 <= current_high)
    ...
    if DEBUG:
        print(f"VALID DSR: {signal_side} - {pattern_type} at {ma_level} - Price: {current_close:.5f}, MA1: {current_ma1:.5f}, MA2: {current_ma2:.5f}")

...
def is_rejection_candle(candle, prev_candle=None):
    """Enhanced rejection candlestick detection"""
    ...
    # Pinbar: significant wick on one side
    if upper_wick > body_size * 2 and upper_wick > lower_wick * 1.5:
        return True, "PINBAR_BEAR"  # Bearish pinbar
    
    if lower_wick > body_size * 2 and lower_wick > upper_wick * 1.5:
        return True, "PINBAR_BULL"  # Bullish pinbar
    ...
    if prev_candle:
        ...
        if (prev_c < prev_o and c > o and  # Previous red, current green
            o <= prev_c and c >= prev_o and  # Current engulfs previous
            body_size > prev_body * 1.2):  # Significantly larger body
            return True, "BULL_ENGULF"
        
        # Bearish engulfing
        if (prev_c > prev_o and c < o and  # Previous green, current red
            o >= prev_c and c <= prev_o and  # Current engulfs previous
            body_size > prev_body * 1.2):  # Significantly larger body
...
def detect_signal(candles, tf, shorthand):
    """Main signal detection using OR logic as specified"""
    n = len(candles)
    if n < MIN_CANDLES:
        return None
    ...
    # Condition 1: Rejection at or near MA1/MA2 (any market state)
    if (is_near_ma(current_low, current_ma1, current_close) or 
        is_near_ma(current_low, current_ma2, current_close)):
        signals.append("BUY")
        reasons.append(f"{pattern_type} rejection near MA1/MA2 support")
    
    if (is_near_ma(current_high, current_ma1, current_close) or 
        is_near_ma(current_high, current_ma2, current_close)):
        signals.append("SELL")
        reasons.append(f"{pattern_type} rejection near MA1/MA2 resistance")
    
    # Condition 2: Trend continuation signals
    if trend == "UPTREND" and (is_near_ma(current_low, current_ma1, current_close) or 
                               is_near_ma(current_low, current_ma2, current_close)):
        signals.append("BUY")
        reasons.append(f"Uptrend continuation - {pattern_type} at MA support")
    
    if trend == "DOWNTREND" and (is_near_ma(current_high, current_ma1, current_close) or 
                                 is_near_ma(current_high, current_ma2, current_close)):
        signals.append("SELL")
        reasons.append(f"Downtrend continuation - {pattern_type} at MA resistance")
    
    # Condition 3: Trend reversal after MA3 break
    if ma3_break == "BREAK_UP" and (is_near_ma(current_low, current_ma1, current_close) or 
                                    is_near_ma(current_low, current_ma2, current_close)):
        signals.append("BUY")
        reasons.append(f"Trend reversal - MA3 break + {pattern_type} retest")
    
    if ma3_break == "BREAK_DOWN" and (is_near_ma(current_high, current_ma1, current_close) or 
                                      is_near_ma(current_high, current_ma2, current_close)):
        signals.append("SELL")
        reasons.append(f"Trend reversal - MA3 break + {pattern_type} retest")
    
    # Return signal if ANY condition is met
    if signals:
        # Use the first signal direction (can be enhanced to prioritize based on strength)
        ...
        return {
            ...
            "ma3": ma3,
            "candles": candles,
            "strength": len(signals)  # Multiple conditions = stronger signal
        }
...
def create_signal_chart(signal_data):
    """Create enhanced chart with signal visualization"""
    ...
    def plot_ma(ma_values, label, color, linewidth=1.5):
        chart_ma = []
    ...
    plot_ma(ma1, "MA1 (SMMA HLC3-9)", "white", 1.5)
    plot_ma(ma2, "MA2 (SMMA Close-19)", "blue", 1.5)
    plot_ma(ma3, "MA3 (SMA MA2-25)", "red", 1.5)
    ...
    if 0 <= signal_chart_idx < len(chart_candles):
        ...
        marker_color = "lime" if signal_data["side"] == "BUY" else "red"
    ...
    ax.set_title(f"{signal_data['symbol']} - {signal_data['side']} Signal", fontsize=14)

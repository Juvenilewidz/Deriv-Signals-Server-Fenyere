# main_1s.py
"""
Lightweight entrypoint specialized for 1s synthetic indices (V75(1s), V100(1s), V150(1s)).
This file is a trimmed/targeted copy of your main runner meant to run side-by-side with the
original main.py. Defaults:
  - ASSETS -> 1HZ75V,1HZ100V,1HZ150V
  - CANDLES_N -> 200
  - TIMEFRAMES -> 5m/10m/15m (300,600,900)

Paste-and-cruise: drop into the repo and run as a separate GitHub Action job:
  python main_1s.py

This file keeps the same analysis and charting logic as your main runner but is quieter
in logs and tuned for reliability on 1s assets.
"""
...
def fetch_candles(symbol: str, granularity: int, count: int = CANDLES_N) -> List[Dict]:
    """
    Quiet robust fetch: snapshot (subscribe=0), 3 retries, returns [] on failure.
    """
    MAX_RETRIES = 3
...
{"updates":[{"pattern":"(?s).*","multiple":false,"replacement":"# main_1s.py\n"""\nLightweight entrypoint specialized for 1s synthetic indices (V75(1s), V100(1s), V150(1s)).\nThis file is a trimmed/targeted copy of your main runner meant to run side-by-side with the\noriginal main.py. Defaults:\n  - ASSETS -> 1HZ75V,1HZ100V,1HZ150V\n  - CANDLES_N -> 200\n  - TIMEFRAMES -> 5m/10m/15m (300,600,900)\n\nPaste-and-cruise: drop into the repo and run as a separate GitHub Action job:\n  python main_1s.py\n\nThis file keeps the same analysis and charting logic as your main runner but is quieter\nin logs and tuned for reliability on 1s assets.\n"""\n\nimport os\nimport json\nimport time\nimport math\nimport tempfile\nimport traceback\nfrom datetime import datetime\nfrom typing import List, Dict, Optional, Tuple\n\nimport websocket\nimport numpy as np\nimport matplotlib\nmatplotlib.use("Agg")\nimport matplotlib.pyplot as plt\nfrom matplotlib.patches import Rectangle\n\n# Try to import bot helpers (these should be in your bot.py)\ntry:\n    from bot import (\n        send_telegram_message,\n        send_single_timeframe_signal,\n        send_strong_signal,\n        send_telegram_photo\n    )\nexcept Exception:\n    # minimal fallback (prints) so file can be tested locally without Telegram\n    def send_telegram_message(token, chat_id, text):\n        print("[TELEGRAM TEXT]", text)\n        return True, "fallback"\n    def send_single_timeframe_signal(symbol, tf, direction, reason, chart_path=None):\n        print("[SIG]", symbol, tf, direction, reason, chart_path)\n        return True\n    def send_strong_signal(symbol, direction, details, chart_path=None):\n        print("[STRONG]", symbol, direction, details, chart_path)\n        return True\n    def send_telegram_photo(token, chat_id, caption, photo_path):\n        print("[PHOTO]", caption, photo_path)\n        return True, "fallback"\n\n# -------------------------\n# Configuration (specialized for 1s indices)\n# -------------------------\nDEBUG = os.getenv("DEBUG", "0") in ("1", "true", "True", "yes")\n\nDERIV_API_KEY = os.getenv("DERIV_API_KEY", "").strip()\nDERIV_APP_ID = os.getenv("DERIV_APP_ID", "1089").strip()\nDERIV_WS_URL = f"wss://ws.derivws.com/websockets/v3?app_id={DERIV_APP_ID}"\n\nTELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "").strip()\nTELEGRAM_CHAT_ID   = os.getenv("TELEGRAM_CHAT_ID", "").strip()\n\nif not DERIV_API_KEY:\n    raise RuntimeError("Missing DERIV_API_KEY env var")\n\n# only 1s assets by default (can be overridden by ASSETS env)\nASSETS = os.getenv("ASSETS", "1HZ75V,1HZ100V,1HZ150V").split(",")\nTIMEFRAMES = [300, 600, 900]  # seconds: 5m, 10m, 15m\n\n# Charting/history tuned for 1s assets\nCANDLES_N = int(os.getenv("CANDLES_N", "200"))  # how many candles to fetch (history)\nLAST_N_CHART = int(os.getenv("LAST_N_CHART", "220"))\nPAD_CANDLES = int(os.getenv("PAD_CANDLES", "10"))\n\n# Drawing\nCANDLE_WIDTH = float(os.getenv("CANDLE_WIDTH", "0.35"))\n\n# Dispatch / cooldown\nMAX_SIGNALS_PER_RUN = int(os.getenv("MAX_SIGNALS_PER_RUN", "3"))\nALERT_COOLDOWN_SECS = int(os.getenv("ALERT_COOLDOWN_SECS", "600"))\n\n# Heartbeat\nHEARTBEAT_INTERVAL_HOURS = float(os.getenv("HEARTBEAT_INTERVAL_HOURS", "0"))\n\n# Persist files (tmp)\nTMPDIR = tempfile.gettempdir()\nCACHE_FILE = os.path.join(TMPDIR, "fenyere_last_sent_1s.json")\nHEART_FILE = os.path.join(TMPDIR, "fenyere_last_heartbeat_1s.json")\n\n# In-memory cache\nlast_sent_cache: Dict[str, Dict] = {}\n\n\ndef log(*args, **kwargs):\n    if DEBUG:\n        ts = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")\n        print("[", ts, "]", *args, **kwargs)\n\n# -------------------------\n# Persistence helpers\n# -------------------------\ndef load_cache():\n    global last_sent_cache\n    try:\n        if os.path.exists(CACHE_FILE):\n            with open(CACHE_FILE, "r") as f:\n                last_sent_cache = json.load(f)\n        else:\n            last_sent_cache = {}\n    except Exception:\n        last_sent_cache = {}\n\ndef save_cache():\n    try:\n        with open(CACHE_FILE, "w") as f:\n            json.dump(last_sent_cache, f)\n    except Exception:\n        pass\n\ndef can_send(symbol: str, direction: str, tf: int) -> bool:\n    rec = last_sent_cache.get(symbol)\n    now = int(time.time())\n    if rec:\n        if rec.get("direction") == direction and (now - rec.get("ts", 0)) < ALERT_COOLDOWN_SECS:\n            return False\n    return True\n\ndef mark_sent(symbol: str, direction: str, tf: int):\n    last_sent_cache[symbol] = {"direction": direction, "tf": tf, "ts": int(time.time())}\n    save_cache()\n\n# -------------------------\n# MA implementations\n# -------------------------\ndef smma(series: List[float], period: int) -> List[Optional[float]]:\n    n = len(series)\n    if n == 0 or period <= 0:\n        return []\n    if n < period:\n        return [None] * n\n    seed = sum(series[:period]) / period\n    out = [None] * (period - 1)\n    out.append(seed)\n    prev = seed\n    for i in range(period, n):\n        prev = (prev * (period - 1) + float(series[i])) / period\n        out.append(prev)\n    return out\n\n\ndef sma(series: List[float], period: int) -> List[Optional[float]]:\n    n = len(series)\n    if period <= 0 or n < period:\n        return [None] * n\n    out = [None] * (period - 1)\n    for i in range(period - 1, n):\n        out.append(sum(series[i - period + 1:i + 1]) / period)\n    return out\n\n\ndef compute_mas_for_chart(candles: List[Dict]) -> Tuple[List[Optional[float]], List[Optional[float]], List[Optional[float]]]:\n    closes  = [c["close"] for c in candles]\n    highs   = [c["high"]  for c in candles]\n    lows    = [c["low"]   for c in candles]\n    hlc3    = [(h + l + c) / 3.0 for h, l, c in zip(highs, lows, closes)]\n    ma1 = smma(hlc3, 9)\n    ma2 = smma(closes, 19)\n    ma2_vals = [v for v in ma2 if v is not None]\n    if len(ma2_vals) >= 25:\n        ma3_raw = sma(ma2_vals, 25)\n    else:\n        ma3_raw = [None] * len(ma2_vals)\n    ma3 = []\n    j = 0\n    for v in ma2:\n        if v is None:\n            ma3.append(None

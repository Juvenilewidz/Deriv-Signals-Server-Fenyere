#!/usr/bin/env python3
# main_1s.py - updated: OR-signal logic (any condition fires). Text-first alerts, chart optional.
...
# Try import bot helpers; fallback to prints/requests
try:
    from bot import send_telegram_message, send_single_timeframe_signal, send_telegram_photo
except Exception:
    try:
        import requests
    except Exception:
...
def already_processed(symbol: str, candle_epoch: int) -> bool:
...
# -------------------------
# MA & candles helpers (same as main)
# -------------------------
...
def fetch_candles(symbol: str, granularity: int, count: int = CANDLES_N) -> List[Dict]:
    attempts = [count, max(100, count // 2), 50, 25]
    for c in attempts:
            ...
            try:
                ...
                if isinstance(resp, dict) and resp.get("candles"):
                    ...
                    for cc in resp["candles"]:
                        try:
                            parsed.append({"epoch": int(cc["epoch"]), "open": float(cc["open"]), "high": float(cc["high"]), "low": float(cc["low"]), "close": float(cc["close"])})
                        except Exception:
                            continue
                    parsed.sort(key=lambda x: x["epoch"])
                    return parsed
            except Exception as e:
                log(f"fetch error {symbol}@{granularity}s count={c}: {e}")
            finally:
                try:
                    if ws: ws.close()
                except: pass
...
def make_chart(candles: List[Dict], ma1: List[Optional[float]], ma2: List[Optional[float]], ma3: List[Optional[float]],
    ...
    try:
        ...
        show_n = min(last_n, total); start = max(0, total - show_n)
        chosen = candles[start: total]
        xs = [datetime.utcfromtimestamp(c["epoch"]) for c in chosen]
        opens = [c["open"] for c in chosen]; highs = [c["high"] for c in chosen]; lows = [c["low"] for c in chosen]; closes = [c["close"] for c in chosen]
        ...
        for i, (o,h,l,c) in enumerate(zip(opens, highs, lows, closes)):
            ...
            lower = min(o,c); height = max(1e-9, abs(c-o))
        ...
        try:
            safe_plot(ma1_vals, "MA1"); safe_plot(ma2_vals, "MA2"); safe_plot(ma3_vals, "MA3"); ax.legend(loc="upper left", fontsize=8)
        except Exception:
            pass
        if start <= rej_index < total:
            idx = rej_index - start; price = chosen[idx]["close"]
            marker_color = "red" if ("SELL" in reason or "sell" in reason.lower()) else "green"
            ax.scatter([idx],[price], marker="v" if "SELL" in reason else "^", color=marker_color, s=120, zorder=6, edgecolors="black")
        ax.set_xlim(-1, len(chosen)-1 + pad)
        ymin, ymax = min(lows), max(highs); pad_y = (ymax-ymin)*0.08 if ymax>ymin else 1e-6; ax.set_ylim(ymin-pad_y, ymax+pad_y)
        step = max(1, len(chosen)//8); ax.set_xticks(range(0,len(chosen),step))
...
# -------------------------
# Pattern detection & OR signal rules (same families as main)
# -------------------------
def candle_bits(candle: Dict, prev: Optional[Dict] = None) -> Dict:
    ...
    if prev:
        ...
        if pc < po and c > o and o <= pc and c >= po: engulf_bull = True
        if pc > po and c < o and o >= pc and c <= po: engulf_bear = True
...
def is_near(price, ma_value, atr, factor=0.6):
    if ma_value is None: return False
    return abs(price - ma_value) <= max(atr*factor, 0.001*price)

def detect_rejection(candles, idx, ma1, ma2):
    if idx <= 0 or idx >= len(candles): return False, "", ""
    candle = candles[idx]; prev = candles[idx-1] if idx-1 >=0 else None
    bits = candle_bits(candle, prev); atr = compute_atr(candles[:idx+1])
    close = candle["close"]; ma1v = ma1[idx] if idx < len(ma1) else None; ma2v = ma2[idx] if idx < len(ma2) else None
    near1 = is_near(close, ma1v, atr); near2 = is_near(close, ma2v, atr)
...
def detect_recent_ma3_cross(candles, ma3, lookback=8):
    n = len(candles)
    for i in range(max(1, n-lookback), n):
        if i-1 < 0: continue
        prevc, curc = candles[i-1]["close"], candles[i]["close"]
...
# -------------------------
# analyze symbol (OR semantics)
# -------------------------
def analyze_symbol(symbol: str, tf: int = 300):
    candles = fetch_candles(symbol, tf, count=CANDLES_N)
    if not candles or len(candles) < 10:
        log(f"[{symbol}] insufficient candles ({len(candles) if candles else 0})"); return None
    ma1, ma2, ma3 = compute_mas_for_chart(candles)
    atr = compute_atr(candles)
    idx = len(candles)-1; epoch = candles[idx]["epoch"]
    if already_processed(symbol, epoch): log(f"[{symbol}] epoch {epoch} processed"); return None

    # A) rejection at MA
    rej_ok, rej_reason, rej_dir = detect_rejection(candles, idx, ma1, ma2)
    # B) MA3 break + retest
    cross = detect_recent_ma3_cross(candles, ma3, lookback=8)
    cross_ok = False; cross_reason = ""
    if cross:
        cross_idx, cross_dir = cross
        if cross_idx < idx:
            r_ok, r_reason, r_dir = detect_rejection(candles, idx, ma1, ma2)
            if r_ok and r_dir == cross_dir:
                cross_ok = True; cross_reason = f"MA3-break+retest: {r_reason}"
    # C) continuation (MA alignment + rejection)
    aligned_ok = False; cont_reason = ""
    try:
    ...
    if (aligned_up or aligned_down):
        r_ok, r_reason, r_dir = detect_rejection(candles, idx, ma1, ma2)
        if r_ok:
            aligned_ok = True; cont_reason = f"Continuation (MA alignment) - {r_reason}"

    # OR: if any true, we produce signal
    reasons=[]; direction=None
    if rej_ok: reasons.append(rej_reason); direction = direction or rej_dir
    if cross_ok: reasons.append(cross_reason); direction = direction or cross_dir
    if aligned_ok: reasons.append(cont_reason); direction = direction or r_dir

    if not reasons:
        mark_processed(symbol, epoch); return None
...
# Sleep window helper
# -------------------------
def in_sleep_window(sleep_window: str, tz_offset: str = "+02:00") -> bool:
    if not sleep_window: return False
    try:
        start_s, end_s = sleep_window.split("-")
        sign = 1 if tz_offset[0] != "-" else -1
        tz_h = int(tz_offset[1:3]) * sign; tz_m = int(tz_offset[4:6]) * sign if len(tz_offset)>=6 else 0
        offset = timedelta(hours=tz_h, minutes=tz_m)
        now_utc = datetime.utcnow(); local_now = now_utc + offset
        sh, sm = [int(x) for x in start_s.split(":")]; eh, em = [int(x) for x in end_s.split(":")]
        start_dt = local_now.replace(hour=sh, minute=sm, second=0, microsecond=0); end_dt = local_now.replace(hour=eh, minute=em, second=0, microsecond=0)
        if start_dt < end_dt: return start_dt <= local_now <= end_dt
        else: return local_now >= start_dt or local_now <= end_dt
    except Exception:
        return False
...
def run_once():
    load_cache()
    if in_sleep_window(SLEEP_WINDOW, TZ_OFFSET): log("Within sleep window -> skip"); return
    if not DISABLE_HEARTBEAT:
        try: send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, "main_1s.py heartbeat")
        except: pass
    ...
    for sym in ASSETS:
        ...
        try:
            ...
            if last and int(last.get("epoch",0)) >= int(sig["epoch"]):
                mark_processed(sym, sig["epoch"]); continue
            if not can_send_cooldown(sym, sig["direction"], sig["tf"]):
                mark_processed(sym, sig["epoch"]); continue
            txt = f"🔔 {sig['symbol']} | {sig['tf']//60}m | {sig['direction']}\nReason: {sig['reason']}\nPrice: {sig['price']}"
            try:
                send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, txt)
                if sig.get("chart"): send_telegram_photo(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, txt, sig["chart"])
            except Exception:
                log("send fail")
            mark_sent(sym, sig['direction'], sig['tf'], sig['epoch'])
            mark_processed(sym, sig['epoch'])
            signals_sent += 1
            reports.append(f"{sym} SENT {sig['direction']}")
        except Exception as e:
            log(f"Error {sym}: {e}")
            if DEBUG: traceback.print_exc()
    save_cache(); log("Run complete. reports:", reports)
